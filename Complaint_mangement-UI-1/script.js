...
// ─── ROLE CONFIG ──────────────────────────────────────────────────────────────
let userRole = localStorage.getItem('clos-role') || 'ADMIN';
...
roleSwitcher.addEventListener('click', function() {
  ...
  localStorage.setItem('clos-role', userRole);
  currentUser.role  = userRole;
  currentUser.name  = userRole === 'ADMIN' ? 'Admin' : 'John Doe';
  currentUser.email = userRole === 'ADMIN' ? 'admin@complaintos.com' : 'john@example.com';
  applyRoleUI();
  showToast('Switched to ' + userRole + ' role', 'info');
  var activeEl = document.querySelector('.section.active');
  if (activeEl) showSection(activeEl.id.replace('section-', ''));
});
